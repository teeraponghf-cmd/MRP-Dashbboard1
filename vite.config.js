...
const SAMPLE_ACTUAL_CONSUMPTION = [
  // รูปแบบ Week แบบ Calendar Week (CW) ย้อนหลัง
  { item: "BIKE-100", week: "26CW27", quantity: 20 },
  { item: "BIKE-100", week: "26CW28", quantity: 22 },
  { item: "BIKE-100", week: "26CW29", quantity: 28 }, // Over-consumption
  ...
  { item: "WHEEL-ASM", week: "26CW29", quantity: 36 }, // Under-consumption
...
function runMRP({ bom, inventory, demand, poPending, git, actualConsumption, batches, horizon, historyWeeks }) {
  ...
  order.forEach((item) => {
    ...
    // แก้ไข: กระจายแผนไปยังชิ้นส่วนลูก รวมถึงแผนในอดีต (เพื่อทำ Baseline สร้าง Variance)
    ...
    kids.forEach(({ component, qty_per }) => {
      ...
      for (let i = 0; i < totalCols; i++) {
        if (i < HW) {
          // สัปดาห์อดีต: ดึง Gross Req จากแม่มาตรงๆ เป็นแผนของตัวลูก
          grossReq[component][i] += (grossReq[item][i] || 0) * qty_per;
        } else {
          // สัปดาห์อนาคต: ดึงจาก Planned Release ตามวงจร MRP ปกติ
