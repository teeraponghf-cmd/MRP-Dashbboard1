...
function ProjectGroupRow({ project, items, records, childrenOf, selected, onSelect, onlyWithOrders, subtreeOrderMap, forceOpen, clearForce }) {
  const [open, setOpen] = useState(true);
  const effectiveOpen = forceOpen !== null ? forceOpen : open;
...
function ProjectGroupTree({ groups, records, childrenOf, selected, onSelect, onlyWithOrders, subtreeOrderMap, forceOpen, clearForce }) {
  return (
    <div>
      {groups.map((g) => (
...
export default function MRPDashboard() {
  ...
  const [isReadOnly] = useState(() => {
    try {
      return typeof window !== "undefined" && new URLSearchParams(window.location.search).get("view") === "1";
...
const [hydrating, setHydrating] = useState(true);
  ...
  const bomRows = useMemo(() => {
    ...
    Object.entries(childrenOf).forEach(([parent, kids]) => {
      ...
      kids.forEach(({ component, qty_per }) => {
        ...
        rows.push({
          project: parent.slice(0, 3).toUpperCase(),
          parent_item: parent,
  ...
  const bomProjectOptions = useMemo(() => Array.from(new Set(bomRows.map((r) => r.project))).sort(), [bomRows]);
  ...
  <div style={{ padding: "10px 14px" }}>
          ...
          {isReadOnly ? (
            <div style={{
              display: "flex", alignItems: "center", gap: 4, marginTop: 4, width: "fit-content",
              fontFamily: "'IBM Plex Mono', monospace", fontSize: 9.5, fontWeight: 600, color: COLORS.amber,
              border: `1px solid ${COLORS.amber}`, background: "#FEF3C7", padding: "2px 8px", borderRadius: COLORS.radiusSm,
            }}>{"\u{1F512}"} view-only mode</div>
          ) : (
            <>
              ...
              <button onClick={copyViewOnlyLink} style={{
                display: "flex", alignItems: "center", gap: 4, cursor: "pointer", marginTop: 4,
                fontFamily: "'IBM Plex Mono', monospace", fontSize: 9.5, fontWeight: 600, color: shareLinkCopied ? "#fff" : COLORS.moss,
                border: `1px solid ${COLORS.moss}`, background: shareLinkCopied ? COLORS.moss : "transparent", padding: "3px 8px", borderRadius: COLORS.radiusSm,
              }}>{"\u{1F517}"} {shareLinkCopied ? "copied!" : "copy view-only link"}</button>
            </>
          )}
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
        <button onClick={() => setShowBOM((s) => !s)} style={{
          display: "flex", alignItems: "center", gap: 6, cursor: "pointer",
          ...
          <ClipboardList size={13} /> {showBOM ? "\u2190 back to dashboard" : "\u{1F4CB} view full BOM"}
        </button>
      </div>
          ...
          <div style={{ padding: "6px 2px", maxHeight: 480, overflowY: "auto" }}>
            {searchResults ? (
              ...
              <ProjectGroupTree groups={projectGroups} records={records} childrenOf={childrenOf} selected={selected} onSelect={handleSelectItem}
                onlyWithOrders={onlyWithOrders} subtreeOrderMap={subtreeOrderMap} forceOpen={forceOpen} clearForce={() => setForceOpen(null)} />
            ) : (
              <>
