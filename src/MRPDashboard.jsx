...
const [hydrating, setHydrating] = useState(true);
  ...
  const updateShipmentTracking = (key, patch) => {
    setShipmentTracking((prev) => ({
      ...prev,
      [key]: { ...(prev[key] || { status: "not_shipped", actualArrivalDate: "", etd: "" }), ...patch },
    }));
  };
  ...
  const shipmentRows = useMemo(() => {
    ...
    order.forEach((item) => {
      ...
      rec.poPendingDetails.forEach((p) => {
        ...
        const tracking = shipmentTracking[key] || { status: "not_shipped", actualArrivalDate: "", etd: "" };
        // ถ้ากรอก ETD (วันที่ของออกจริง) ไว้แล้ว ให้คำนวณ ETA จาก ETD + lead time แทนการนับจากวันนี้
        const etdDate = tracking.etd ? new Date(tracking.etd + "T00:00:00Z") : null;
        const baseDate = etdDate && !isNaN(etdDate) ? etdDate : new Date();
        const etaDate = new Date(baseDate.getTime() + chosenWeeks * 7 * 86400000);
            ...
            <table style={{ borderCollapse: "collapse", width: "100%", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11.5 }}>
              <thead>
                <tr style={{ position: "sticky", top: 0, background: COLORS.card, zIndex: 1 }}>
                  {["ITEM", "PO #", "QTY", "DESCRIPTION", "VENDOR", "COVERAGE (WK)", "RECOMMENDED", "METHOD", "ETD", "ETA", "ARRIVED?", "TRACKING STATUS", "ACTUAL ARRIVAL"].map((h) => (
                    ...
                    <tr key={r.key} onClick={() => handleSelectItem(r.item)} style={{ cursor: "pointer" }}>
                      ...
                      <td style={{ padding: "4px 8px", borderBottom: `1px solid ${COLORS.paperLine}` }} onClick={(e) => e.stopPropagation()}>
                        <input
                          ...
                          value={r.etd}
                          disabled={isReadOnly}
                          title="ETD — วันที่ของออกจากต้นทางจริง (ถ้ากรอกไว้ ETA จะคำนวณจากวันนี้แทนวันนี้)"
                          onChange={(e) => updateShipmentTracking(r.key, { etd: e.target.value })}
                          style={{
                            fontFamily: "'IBM Plex Mono', monospace", fontSize: 10.5, padding: "3px 6px",
                            border: `1px solid ${r.etd ? COLORS.steel : COLORS.paperLine}`, borderRadius: COLORS.radiusSm,
