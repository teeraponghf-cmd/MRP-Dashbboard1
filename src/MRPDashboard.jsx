...
const poDetailsByItem = {};
  ...
  order.forEach((item) => {
    ...
    // ค่าเฉลี่ยจาก Gross Requirements (BOM/Demand) ในช่วงเดียวกันแทน กันไม่ให้ coverage ขึ้น "no data"
    // (ไม่กระทบกับตัวเลข "PAST Xw AVG" ที่โชว์ในหัวตาราง — อันนั้นยังคงอ้างอิงยอดเบิกจริงเท่านั้นเหมือนเดิม)
    const pastGrossTotalForAvg = gr.slice(0, HW).reduce((a, b) => a + (b || 0), 0);
    const pastGrossAvgForCoverage = HW > 0 ? pastGrossTotalForAvg / HW : 0;
    const avgConsumptionForCoverage = pastActualAvg > 0 ? pastActualAvg : pastGrossAvgForCoverage;
