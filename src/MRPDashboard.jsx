...
const poDetailsByItem = {};
  ...
  order.forEach((item) => {
    ...
    // สำหรับ coverage: ถ้าไม่มีข้อมูล actual consumption เลย (pastActualAvg = 0) ให้ fallback ไปใช้
    // ค่าเฉลี่ยจาก Gross Requirements (BOM/Demand) แทน กันไม่ให้ coverage ขึ้น "no data"
    // ใช้เฉลี่ยจากทั้ง "ประวัติ + demand ในอนาคตทั้ง horizon" ไม่ใช่แค่ประวัติอย่างเดียว
    // เพราะบาง item ประวัติมีน้อย/เป็น 0 แต่ demand ที่กำลังจะมาถึงสูงขึ้นมาก การดูแค่ประวัติจะทำให้ coverage ดูดีเกินจริง
    ...
    const avgConsumptionForCoverage = pastActualAvg > 0 ? pastActualAvg : grAllAvgForCoverage;
  ...
  const coverageRows = useMemo(() => {
    const rows = order.map((item) => {
      ...
      return {
        ...
        is_bom_estimate: rec.pastActualAvg === 0 && avg > 0, // ไม่มียอดเบิกจริง ใช้ค่าเฉลี่ยจาก BOM/Demand แทน
        lead_time_weeks: rec.leadTime,
            ...
            {coverageRowsFiltered.map((r) => {
              ...
              return (
                <div key={r.item} onClick={() => handleSelectItem(r.item)} style={{
                  ...
                  <div style={{ width: 90, textAlign: "right", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, fontWeight: 700, color: barColor === COLORS.paperLine ? COLORS.inkSoft : barColor }} title={r.is_bom_estimate ? "ไม่มียอดเบิกจริงในไฟล์ Actual Consumption — คำนวณ coverage จากค่าเฉลี่ย Gross Requirements (BOM/Demand) แทน" : undefined}>
