                ...
                return (
                    <td key={i} style={{
                      ...
                      {isEditable ? (
                        ...
                        ) : ((r.kind === "ssline" || r.kind === "poh") ? Math.round(v) : (r.kind === "gr" && rec.isForecast && rec.isForecast[i] && v ? (
                          <span title="ค่าพยากรณ์ (forecast) — Demand Schedule ยังไม่มีข้อมูลสัปดาห์นี้ ใช้ค่าเฉลี่ยจากประวัติแทน">
                            {Math.round(v)}<sup style={{ fontSize: 8, color: "#6D28D9" }}>F</sup>
                          </span>
                        ) : (v ? Math.round(v) : "—"))))
                      )}
                    </td>
