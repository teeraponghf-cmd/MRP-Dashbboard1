...
function MRPDashboardInner({ role, onLogout }) {
  ...
  const [shipmentMethod, setShipmentMethod] = useState({}); // item -> "sea" | "air" (override จากค่าที่ระบบแนะนำ)
  const SEA_WEEKS = 10;
  const AIR_WEEKS = 2;
...
const [hydrating, setHydrating] = useState(true);
  ...
  useEffect(() => {
    (async () => {
      const [savedOrderStatus, savedPlanOverrides, savedReceiptOverrides, savedHorizon, savedHistoryWeeks, savedPoOverrides, savedDraftRefs, savedShipmentMethod] = await Promise.all([
  ...
  const clearSavedSettings = async () => {
    await storageClearAll(["orderStatus", "planOverrides", "receiptOverrides", "horizon", "historyWeeks", "poOverrides", "draftRefs", "shipmentMethod"]);
    setOrderStatus({});
    setPlanOverrides({});
    setReceiptOverrides({});
    setDraftRefs({});
  ...
  const shipmentRows = useMemo(() => {
    return order
      .filter((item) => {
        const rec = records[item];
        return rec.plannedRelease.some((v) => v > 0) || rec.projOnHand[historyWeeks] < rec.safety;
      })
      .map((item) => {
        const rec = records[item];
        const avg = rec.pastActualAvg;
        ...
        const chosen = shipmentMethod[item] || (recommended === "critical" ? "air" : recommended);
  ...
  }, [order, records, historyWeeks, shipmentMethod]);
  ...
  const setShipmentMethodForItem = (item, method) => {
    setShipmentMethod((prev) => ({ ...prev, [item]: method }));
  };

  const exportShipment = () => {
    downloadCSV(`import_shipment_plan_${new Date().toISOString().slice(0, 10)}.csv`, shipmentRowsFiltered.map((r) => ({
      item: r.item, description: r.description, unit: r.unit, vendor: r.vendor,
      ...
      {showShipment ? (
        <div style={{
            ...
            <table style={{ borderCollapse: "collapse", width: "100%", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11.5 }}>
              <thead>
                <tr style={{ position: "sticky", top: 0, background: COLORS.card, zIndex: 1 }}>
                  {["ITEM", "DESCRIPTION", "VENDOR", "ON HAND", "COVERAGE (WK)", "RECOMMENDED", "METHOD", "ETA", "STATUS"].map((h) => (
              ...
              <tbody>
                {shipmentRowsFiltered.map((r) => {
                  ...
                  return (
                    <tr key={r.item} onClick={() => handleSelectItem(r.item)} style={{ cursor: "pointer" }}>
                      ...
                      <td style={{ padding: "6px 10px", borderBottom: `1px solid ${COLORS.paperLine}`, color: COLORS.inkSoft, whiteSpace: "nowrap", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis" }}>{r.description}</td>
                      <td style={{ padding: "6px 10px", borderBottom: `1px solid ${COLORS.paperLine}`, color: COLORS.inkSoft, whiteSpace: "nowrap" }}>{r.vendor || "\u2014"}</td>
                      <td style={{ padding: "6px 10px", borderBottom: `1px solid ${COLORS.paperLine}`, textAlign: "right" }}>{r.onHand} {r.unit}</td>
                      ...
                      <td style={{ padding: "4px 8px", borderBottom: `1px solid ${COLORS.paperLine}` }} onClick={(e) => e.stopPropagation()}>
                        <select
                          value={r.chosen}
                          disabled={isReadOnly}
                          onChange={(e) => setShipmentMethodForItem(r.item, e.target.value)}
                          style={{
                      ...
                      <td style={{ padding: "6px 10px", borderBottom: `1px solid ${COLORS.paperLine}`, whiteSpace: "nowrap" }}>
                        {r.willArriveInTime ? (
                          ...
                          <span style={{ color: COLORS.rust, fontWeight: 600, fontSize: 10.5 }}>{"\u2715"} ไม่ทัน แม้เลือกวิธีนี้</span>
                        )}
                      </td>
                    </tr>
                  );
                ...
                {shipmentRowsFiltered.length === 0 && (
                  <tr><td colSpan={9} style={{ padding: 20, textAlign: "center", color: COLORS.inkSoft }}>ไม่มี item ที่ต้องวางแผนนำเข้าในตอนนี้</td></tr>
                )}
              </tbody>
            </table>
          </div>
          <div style={{ padding: "8px 14px", borderTop: `1px solid ${COLORS.paperLine}`, fontFamily: "'IBM Plex Mono', monospace", fontSize: 9.5, color: COLORS.inkSoft }}>
            RECOMMENDED คำนวณจาก coverage (on-hand \u00f7 อัตราเบิกใช้เฉลี่ย) เทียบกับ lead time ของ Sea/Air — {"\u26A0\uFE0F"} CRITICAL หมายถึง coverage ต่ำกว่า Air lead time แล้ว (แม้ส่งทางอากาศก็อาจไม่ทัน ควรเร่งเป็นพิเศษ) — เปลี่ยนวิธีขนส่งได้ที่ช่อง METHOD จะบันทึกไว้ให้อัตโนมัติ
          </div>
        </div>
      ) : showCoverage ? (
        <div style={{
