...
function RecordGrid({ rec, weeks, weekLabels, weekDates, historyWeeks, onAdjustPlan, onResetPlanOverride, onAdjustReceipt, onResetReceiptOverride, onAdjustPOQty, poOriginalQtyMap, onResetPOQty, onAdjustPOWeek, onResetPOWeek, poOriginalMap, planOverrides, receiptOverrides }) {
  ...
  return (
    <div style={{ border: `1px solid ${COLORS.ink}`, background: COLORS.card }}>
      {/* title block */}
      <div style={{
        display: "grid", gridTemplateColumns: "repeat(8, 1fr)", borderBottom: `1px solid ${COLORS.ink}`,
        ...
        {[
          ...
          <div key={k} style={{
            padding: "6px 10px", borderRight: i < 7 ? `1px solid ${COLORS.paperLine}` : "none",
            background: k === "EXPIRY" && rec.expired ? COLORS.rust : k === "EXPIRY" && rec.expiringSoon ? "#F3DDBC" : k.includes("AVG") ? "#E3E9D6" : k.includes("ON HAND") ? "#DCE7EE" : "transparent",
          }}>
            <div style={{ color: k === "EXPIRY" && rec.expired ? "#F6D9D3" : k.includes("AVG") ? COLORS.moss : k.includes("ON HAND") ? COLORS.steelDeep : COLORS.inkSoft, letterSpacing: "0.05em" }}>{k}</div>
            ...
            {rows.map((r) => {
              ...
              <tr key={r.kind}>
                ...
                {r.data.map((v, i) => {
                  ...
                  if (r.kind === "consumption" && rec.consumptionFactor !== 1 && v > 0) { bg = "#DCE7EE"; color = COLORS.steelDeep; }
                  if (r.kind === "actual" && v > 0) { bg = "#E9E4F0"; color = "#5A4A78"; }
                  if (r.kind === "ssline" && v !== null) {
                    if (v < 0) { bg = COLORS.rust; color = "#fff"; }
                    else if (v < rec.safety * 0.25) { bg = "#F3DDBC"; color = COLORS.amber; }
                    else { bg = "#E3E9D6"; color = COLORS.moss; }
                  }
      ...
      <div style={{
        ...
        <span style={{
          ...
          {onlySelected && selectedItem && (
            <span style={{
              fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, fontWeight: 400, textTransform: "none",
              color: COLORS.steel, background: "#DCE7EE", border: `1px solid ${COLORS.steel}`, padding: "1px 6px",
            }}>{selectedItem}</span>
          )}
        </span>
...
const [hydrating, setHydrating] = useState(true);
  ...
  useEffect(() => {
    (async () => {
      const [savedOrderStatus, savedPlanOverrides, savedReceiptOverrides, savedHorizon, savedHistoryWeeks, savedPoOverrides] = await Promise.all([
  ...
  const clearSavedSettings = async () => {
    await storageClearAll(["orderStatus", "planOverrides", "receiptOverrides", "horizon", "historyWeeks", "poOverrides"]);
    setOrderStatus({});
    setPlanOverrides({});
    setReceiptOverrides({});
    setHorizon(12);
  ...
  <div style={{ padding: "10px 14px" }}>
          ...
          <button onClick={() => { if (window.confirm("ล้างค่า order status / ค่าที่ปรับเอง / horizon ที่บันทึกไว้ทั้งหมด?")) clearSavedSettings(); }} style={{
            ...
            border: `1px solid ${COLORS.paperLine}`, background: "transparent", padding: "2px 6px",
