...
const [hydrating, setHydrating] = useState(true);
  ...
  // ---------------------------------------------------------
  // โหลดค่าที่เคยบันทึกไว้กลับมา (order status, ค่าที่ปรับเอง, horizon)
  // ---------------------------------------------------------
  useEffect(() => {
    (async () => {
      const [savedOrderStatus, savedPlanOverrides, savedReceiptOverrides, savedHorizon, savedHistoryWeeks] = await Promise.all([
  ...
  const clearSavedSettings = async () => {
    await storageClearAll(["orderStatus", "planOverrides", "receiptOverrides", "horizon", "historyWeeks"]);
    setOrderStatus({});
    setPlanOverrides({});
    setReceiptOverrides({});
    setHorizon(12);
